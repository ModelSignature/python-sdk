...
class PolicyEnforcer:
    ...
    def enforce_policy(self, token: str) -> PolicyResult:
        ...
        try:
            ...
            if not verification.valid:
                violations.append(
                    f"Token verification failed: {verification.error}"
                )
            ...
            verification = VerificationResult(valid=False, error=error_message)
        ...
        if not allowed and self.config.fail_closed:
            raise PolicyViolationError(
                f"Policy violation: {', '.join(violations)}", violations
            )
    ...
    def _check_token_policy(
        ...
        if self.config.require_bundle_verification:
            if not bundle_check:
                violations.append(
                    "Bundle verification is required but no bundle "
                    "information available"
                )
            elif bundle_check.status != "verified":
                violations.append(
                    f"Bundle verification failed: status is "
                    f"'{bundle_check.status}'"
                )
        ...
        if self.config.allowed_providers:
            if (
                not provider
                or provider.id not in self.config.allowed_providers
            ):
                provider_name = provider.id if provider else 'unknown'
                violations.append(
                    f"Provider '{provider_name}' is not in allowed list"
                )
        ...
        if self.config.allowed_models:
            if not model or model.id not in self.config.allowed_models:
                ...
                violations.append(
                    f"Model '{model_name}' is not in allowed list"
                )
    ...
    def _build_result(
        ...
        return PolicyResult(
            ...
            or VerificationResult(valid=False, error="No verification data"),
        )
...
def create_secure_policy(
    client: ModelSignatureClient, config: Optional[PolicyConfig] = None
) -> PolicyEnforcer:
    """
    ...
    if config:
        ...
        secure_config.require_bundle_verification = (
            config.require_bundle_verification
        )
...
def create_lenient_policy(
    ...
    if config:
        ...
        lenient_config.require_bundle_verification = (
            config.require_bundle_verification
        )
